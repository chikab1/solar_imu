...
- 实时姿态与六轴页面均可选择本地文件夹记录历史 CSV；六轴页面显示加速度、角速度与姿态角三组历史曲线，并显示/记录总加速度。
...
“实时姿态”和“六轴数据”页面各自提供“选择保存文件夹”和“开始记录”按钮。选择文件夹后会记住该目录；记录时只保存成功返回的 `GET_IMU_LIVE` 样本，断开设备或关闭程序会自动关闭文件。

文件按电脑本地日期追加保存，不覆盖同一天已经记录的数据：

```text
attitude_YYYYMMDD.csv
six_axis_YYYYMMDD.csv
```

姿态文件包含带本地 UTC 偏移的采样时间、Pitch 和 Roll（°）。六轴文件还包含加速度 X/Y/Z（mg）、总加速度 `sqrt(x²+y²+z²)`（mg）、角速度 X/Y/Z（°/s）以及 Pitch/Roll（°）。六轴页面的三块历史曲线分别显示：加速度 X/Y/Z/总加速度、角速度 X/Y/Z、姿态角 Pitch/Roll；每组最多保留最近 300 个样本，清空曲线不会删除 CSV 数据。
