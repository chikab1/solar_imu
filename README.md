...
| `cmd_id` | 是 | 单调递增的正整数命令ID；小于或等于上次已执行ID时忽略 |
| `ver` | 是 | 协议版本，当前固定为`1` |
...
3. 校验`cmd_id`、`ver`和所有已提供的配置字段；`cmd_id`必须大于上次已执行ID。任意字段不符合要求时，整条消息都不会应用。
4. 校验通过后一次性保存配置和`cmd_id`到RTC备份寄存器，并立即重新配置IMU唤醒阈值。
5. `sleep`发生变化时清零当前RTC累计时间，下一周期从配置生效时开始计算。
6. Retain消息在设备每次RTC联网时可能再次收到；设备忽略`cmd_id`小于或等于已执行ID的消息，防止重复执行和旧配置回滚。
...
订阅失败、等待超时、publish URC无法解析、Topic不是`device/settings`、IMEI缺失/非法/不匹配、`cmd_id`缺失或非法时，不发送失败ACK，因为设备无法安全地把ACK关联到本条命令。失败校验不会修改当前配置或已执行的`cmd_id`。
