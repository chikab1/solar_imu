...
- `pc_tool` 提供 V1.2.0 Windows 上位机源码；支持3D实时姿态、六轴数据、IMEI读取、用户单位参数设置及写入后自动回读校验。
...
- 修复角度唤醒的来源识别问题，避免 6D 倾角事件被错误同时标记为加速度唤醒。
- 调整上位机功能与交互，包含 3D 姿态预览、4G 网络状态、GPS 上报选项、低功耗待机和操作反馈等改进。
- 上位机界面已预留学校 Logo 位置，并将其用作应用图标资源。
...
| 参数 | 默认值 | 说明 |
|---|---:|---|
| Wake-Up阈值 | 500 mg | 当前默认运动唤醒阈值 |
| Wake-Up持续时间 | 0 / ODR | 不额外延迟；硬件检测到超过阈值的Wake-Up条件后即可触发 |
...
| `ts` | Unix时间戳 |
| `v` | 电池电压，mV |
| `w` | 上报类别：0未知、1 IMU加速度（WAKE-UP）运动事件、2确认倾斜/恢复、3确认倾斜且同时有WAKE-UP、4 RTC、5人工/开机。WU来源直接作为运动事件上报；6D仍要求姿态复核。主动采样模式切换后会丢弃前200ms（20帧）数据。 |
